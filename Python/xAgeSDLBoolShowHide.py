    ...
    def _Setup(self):
        ...
        if sdlName.value:
            ...
            #except KeyError:
                ...
                PtDebugPrint("xAgeSDLBoolShowHide._Setup():\tWARNING: {0}'s SDL variable '{1}' key does not exist!".format(self.sceneobject.getName(), sdlName.value))
                self.sdl_value = defaultValue.value # start at default
            except IndexError:
                PtDebugPrint("xAgeSDLBoolShowHide._Setup():\tWARNING: {0}'s SDL variable '{1}' index does not exist!".format(self.sceneobject.getName(), sdlName.value))
                self.sdl_value = defaultValue.value # start at default
        else:
            self.sdl_value = defaultValue.value # start at default
